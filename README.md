...
`API Key管理` 页面聚焦 API Key 管理：列表、筛选、搜索、刷新、自动刷新、创建/编辑抽屉、资源绑定、额度展示、今日用量和启停/删除操作。

`设置` 页面用于管理临时 Management Key 和模型计费价格，可手动维护价格，也可从 LiteLLM 公开价格表同步模型价格。库存和分组目前主要作为后端管理能力保留，不再作为独立页面展示。
...
```text
/v0/resource/plugins/cpa-toolkit/apikey.html
/v0/resource/plugins/cpa-toolkit/settings.html
```
...
POST                  /v0/management/plugins/cpa-toolkit/prices/sync
...
从 LiteLLM 公开价格表同步价格规则：

```bash
curl -X POST "$CPA/v0/management/plugins/cpa-toolkit/prices/sync" \
  -H "Authorization: Bearer $CPA_MANAGEMENT_KEY" \
  -H "content-type: application/json" \
  -d '{}'
```
